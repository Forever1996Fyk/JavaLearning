...
**`AQS`通过内置的FIFO(First In First Out)同步队列来完成资源获取线程的排队功能, 如果当前线程获取同步状态失败(锁/资源)时,  `AQS`则会将当前线程以及等待状态等信息构造成一个节点(Node)并将其加入同步队列, 同时阻塞当前线程, 当同步状态释放时, 则会把节点中的线程唤醒, 再次尝试获取同步状态。这个同步队列称为`CLH队列`**
