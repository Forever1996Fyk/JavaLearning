...
**CPU就像一座工厂, 时刻在运行。进程就是工厂的车间, 一个车间里可以有很多工人, 他们协同完成一个任务。线程就相当于车间里的工人, 一个进程可以包括多个线程。**

> 这里要注意的是, 计算机分配的是`进程`, 但是CPU分配的是`线程`。所以`线程`是CPU执行的基本单位。

**每个线程都有自己独立的程序计数器和栈区域。**

> - **程序计数器**: <br/>
>   我们知道CPU一般是通过时间片轮转方式轮询占用的, 所以当前线程CPU时间片使用完毕后, 要让出CPU, 等待下一次轮到自己的时候在调用。那么问题来了, 线程是如何知道之前执行到哪了?<br/>
>   程序计数器就是为了记录之前让出CPU时执行到的指令地址, 等下次再执行时就可以从程序计数器中获取之前的执行地址, 继续向下执行。(但是程序计数器不会记录native方法执行的地址, 它只记录java代码执行的地址)<br/>
> - **栈区域**: <br/>
>   每一个线程都是自己的私有栈区域用于存储线程的局部变量。其他线程无法访问, 这也是为什么局部变量是没有线程安全问题的。<br/>
> - **堆内存**: <br/>
>   这是一块比较大的内存。一般用于存放new创建的对象的实例。这是块共享的区域, 所有线程都能访问。<br/>
> - **方法区**: <br/>
>   这是存储JVM加载的类, 常量和静态变量的区域。这也是共享区域， 所有线程都能访问。
